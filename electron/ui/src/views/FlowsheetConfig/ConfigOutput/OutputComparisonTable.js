...
export default function OutputComparisonTable(props) {
    ...
    const organizeVariables = () => {
        ...
        for (const bvars of historyData) {
            ...
            const outputs = getOutputs(bvars.data)
        ...
        return <FormControl>
            ...
            >
                {historyDataOrganized.map((value, ind) => {
                    return <MenuItem key={value + ind}
                                     value={ind}>{value.name}</MenuItem>
                })}
            </Select>
        </FormControl>
    }
                ...
                if (historyDataOrganized[leftConfigIndex].data[category].variables.length > 0) {
                    return (<Fragment key={category + index}>
                            <TableRow key={category + " " + index}>
                                <TableCell
                                    rowSpan={historyDataOrganized[leftConfigIndex].data[category].variables.length + 1}>
                                    <b>{category}</b>
                                </TableCell>
                            </TableRow>
                ...
                <Toolbar spacing={2}>
                    ...
                    <Stack direction="row" spacing={2}>
                        <Button variant="outlined" startIcon={<DownloadIcon/>}
                                onClick={downloadSheet}>Download Results</Button>
                    </Stack>
                </Toolbar>
            </Grid>
            <Paper>
                <Table style={{border: "1px solid #ddd"}}
                    ...
                    <TableHead>
                        <TableRow key="tablehead">
                            ...
                            <TableCell
                                style={{width: '25%'}}>{renderConfigurationSelect(0)}</TableCell>
                            <TableCell
                                style={{width: '25%'}}>{renderConfigurationSelect(1)}</TableCell>
                            <TableCell style={{width: '15%'}}>Value
