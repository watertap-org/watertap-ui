                ...
                category === "" ?
                            ...
                            {Object.keys(CATEGORIES).map((key) => {
                                ...
                                if ((key === 'Custom Flowsheets' && checkForCustomFlowsheets()) ||
                                    ...
                                    >
                                        ...
                                        <span><IconButton><ArrowRightAltIcon/></IconButton></span>
                                        </TableCell>
                                    </TableRow>
                                )}
                                return result;
                            })}
                            <TableRow sx={styles.listRow}
                                      onClick={() => setCategory("All Flowsheets")}>
                    ...
                    <Table sx={{minWidth: 700}}>
                        <TableHead sx={{margin: "50px"}}>
                            <TableRow>
                                <TableCell>
                                    <IconButton sx={{marginLeft: -5, marginRight: 1}}
                                                onClick={() => setCategory("")}><KeyboardBackspaceIcon/></IconButton>
                                    <span onClick={() => handleSort('name')}
                                          style={{cursor: "pointer"}}>
                            Flowsheet Name
                            <IconButton>{sortKey === "name" && (sortDirection === "ascending" ?
                                <KeyboardArrowDownIcon/> : sortDirection === "descending" &&
                                <KeyboardArrowUpIcon/>)}</IconButton>
                        </span>
                                </TableCell>
                                <TableCell align="right">
                        <span onClick={() => handleSort('last_run')}
                              style={{cursor: "pointer"}}>
                            Last Run
                            {sortKey === "name" &&
                                <span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span>}
                            <IconButton>{sortKey === "last_run" && (sortDirection === "ascending" ?
                                <KeyboardArrowDownIcon/> : sortDirection === "descending" &&
                                <KeyboardArrowUpIcon/>)}</IconButton>
                        </span>
                                </TableCell>
